...
v0.1.2 起，单条与组合默认启用，远程订阅默认自动检测到期／额度。应用启动后检查一次，此后每 30 分钟检查，最多两条远程来源并发，复用现有超时和请求限流。没有新增 systemd 服务或定时备份。手动关闭优先且停止定期检查；自动停用的来源继续检查以便续费／流量重置后恢复。

配置中的 `enabled`、`autoManage` 是兼容旧数据的可选字段。最新检测结果保存在数据目录的 `root.json` 内，按订阅及来源关联，每个来源只保留最新记录；重启后恢复，改名、排序、删除和导入会保持或清理对应状态。认证状态仍使用独立目录。状态读取不向供应商请求，立即检测会绕过旧流量缓存。

从“我的 → 关于 Sub-Store”按现有流程升级，无需重新安装助手。新旧程序继续使用 `2.0` 数据格式，**v0.1.1 及更早版本不会执行启停规则，回退后已停用来源可能重新输出**。网页回退默认保留最新数据，显式恢复快照则恢复快照里的启停配置与检测状态。常态仍是两版程序和一份数据快照。
